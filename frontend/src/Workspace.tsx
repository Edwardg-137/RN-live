...
import { kinds, statuses, type Recording, type Segment, type Speaker, type Transcript, type Snapshot } from './types'
...
export function Workspace({id,onChange,onDeleted,onDirty}:{id:string;onChange:()=>void;onDeleted:()=>void;onDirty:(dirty:boolean)=>void}) {
  ...
  async function load() {
    ...
    setRecord(item); setSpeakers(item.speakers); setTranscript({revision:item.revision,segments:item.segments})
  }
  useEffect(()=>{let cancelled=false; api<Snapshot>(path).then(item=>{if(!cancelled){setRecord(item);setSpeakers(item.speakers);setTranscript({revision:item.revision,segments:item.segments})}}).catch(e=>{if(!cancelled)setError(e.message)}); return()=>{cancelled=true}},[path])
  useEffect(()=>{onDirty(dirty); const warn=(e:BeforeUnloadEvent)=>{if(dirty)e.preventDefault()}; window.addEventListener('beforeunload',warn);return()=>window.removeEventListener('beforeunload',warn)},[dirty,onDirty])
  useEffect(()=>{
    if(!processing)return
    let stopped=false
    const timer=window.setInterval(async()=>{try{const item=await api<Snapshot>(path);if(stopped)return;setRecord(item);if(!['queued','transcribing'].includes(item.status)){setSpeakers(item.speakers);setTranscript({revision:item.revision,segments:item.segments});onChange()}}catch(e){if(!stopped)setError((e as Error).message)}},2000)
    return()=>{stopped=true;window.clearInterval(timer)}
  },[processing,path])
  useEffect(()=>{if(follow&&active.length)rows.current[active[0]]?.scrollIntoView({block:'nearest',behavior:'smooth'})},[activeKey,follow])

  function changeSegment(index:number,change:Partial<Segment>){setTranscript(value=>({...value,segments:value.segments.map((s,i)=>i===index?{...s,...change}:s)}));setDirty(true)}
  ...
  async function save(){setBusy(true);setError('');setNotice('');try{const saved=await api<{revision:number}>(path+'/speakers',json('PUT',{revision:transcript.revision,speakers}));setTranscript(value=>({...value,revision:saved.revision}));const result=await api<Transcript>(path+'/segments',json('PUT',{...transcript,revision:saved.revision}));setTranscript(result);setDirty(false);setNotice('Revisión guardada.');onChange()}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
  function split(index:number){try{const parts=splitSegment(transcript.segments[index],Math.round(time*1000),cursors.current[index]??0);setTranscript(value=>({...value,segments:[...value.segments.slice(0,index),...parts,...value.segments.slice(index+1)]}));setDirty(true)}catch(e){setError((e as Error).message)}}
  ...
  return <>
    ...
    <div className="toolbar"><button disabled={busy||processing||dirty} onClick={()=>{if(transcript.segments.length&&!window.confirm('Volver a transcribir reemplazará la revisión cuando termine. ¿Continuar?'))return;void action('transcribe')}}>Transcribir</button>{processing&&<button disabled={busy} onClick={()=>void action('cancel')}>Cancelar trabajo</button>}<button className="primary" disabled={busy||processing||!dirty} onClick={()=>void save()}>Guardar revisión{dirty?' *':''}</button><button className="danger" disabled={busy} onClick={()=>void remove()}>Eliminar</button></div>
    ...
    <div className="workspace-grid">
      <section className="media-column">
        ...
        <section className="panel"><h2>Quién habla</h2><p className="muted">Las etiquetas separan voces. Puedes asignar nombres y roles.</p><fieldset disabled={busy||processing}>{speakers.map((speaker,i)=><div className="speaker-row" key={speaker.id}><label>Hablante {i+1}<input value={speaker.name} maxLength={100} placeholder={speaker.id} onChange={e=>{setSpeakers(values=>values.map((v,j)=>j===i?{...v,name:e.target.value}:v));setDirty(true)}}/></label><label>Rol<select value={speaker.role} onChange={e=>{setSpeakers(values=>values.map((v,j)=>j===i?{...v,role:e.target.value as Speaker['role']}:v));setDirty(true)}}>{Object.entries({unspecified:'Sin especificar',speaker:'Orador',moderator:'Moderador',interviewer:'Entrevistador',guest:'Invitado'}).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></div>)}<button disabled={speakers.length>=4} onClick={()=>{setSpeakers([...speakers,{id:crypto.randomUUID(),name:'',role:'unspecified'}]);setDirty(true)}}>Añadir hablante</button></fieldset></section>
        <section className="panel next-stage"><p className="eyebrow">PRÓXIMA ETAPA</p><h2>Verificación de afirmaciones</h2><p>La búsqueda de evidencia y el análisis de falacias todavía no están implementados. Esta entrega permite preparar y revisar la grabación.</p></section>
      </section>
      <section className="transcript-panel"><div className="transcript-header"><h2>Transcripción <small>{transcript.segments.length} segmentos</small></h2><label className="inline"><input type="checkbox" checked={follow} onChange={e=>setFollow(e.target.checked)}/>Seguir reproducción</label></div>
        <div className="transcript-scroll">{!transcript.segments.length&&<p className="empty">{processing?'Procesando la grabación. Puedes volver más tarde.':'Solicita la transcripción o añade segmentos manualmente para revisar el contenido.'}</p>}
          {transcript.segments.map((segment,index)=><article ref={node=>{rows.current[index]=node}} key={segment.id??`new-${index}`} className={`segment ${active.includes(index)?'active':''}`}>
            ...
            <fieldset disabled={busy||processing}>
              ...
              <div className="segment-actions"><button title="Pausa el reproductor en el punto de corte y coloca el cursor en el texto" onClick={()=>split(index)}>Dividir aquí</button><button onClick={()=>{setTranscript(value=>({...value,segments:value.segments.filter((_,i)=>i!==index)}));setDirty(true)}}>Quitar segmento</button></div>
            </fieldset>
          </article>)}
        </div><div className="transcript-footer"><button disabled={busy||processing} onClick={()=>{const start=Math.min(Math.round(time*1000),Math.max(0,record.duration_ms-1));setTranscript(value=>({...value,segments:[...value.segments,{start_ms:start,end_ms:Math.min(start+5000,record.duration_ms),text:'',speaker_id:null,reviewed:false}]}));setDirty(true)}}>Añadir segmento en este momento</button></div>
      </section>
    </div>
  </>
}
