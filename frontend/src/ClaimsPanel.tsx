...
import { claimEditBody, claimSpeakers, filterClaims } from './claims'
...
export function ClaimsPanel({run,segments,busy,onStart,onCancel,onSeek,onEdit,onSelect,onDraftDirty=()=>undefined}:Props){
  ...
  return <section className="panel claims-panel" aria-labelledby="claims-title">
    ...
    {run&&(run.status==='completed'||stale)&&<>
      ...
      <div className="claim-filters"><label>Estado<select value={status} onChange={event=>setStatus(event.target.value)}><option value="all">Todos</option>{Object.entries(statusLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Categoría<select value={category} onChange={event=>setCategory(event.target.value)}><option value="all">Todas</option>{Object.entries(categoryLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></div>
      <div className="claims-list">{!claims.length&&<p className="muted">No hay candidatas para los filtros seleccionados.</p>}{claims.map(claim=><article className={`claim-card claim-${claim.status}`} key={claim.id}>
        <div className="claim-heading"><button className="time-link" onClick={()=>onSeek(claim.start_ms)}>{formatTime(claim.start_ms)}–{formatTime(claim.end_ms)} ↗</button><span className="badge">{statusLabels[claim.status]}</span></div>
        ...
        {editing===claim.id&&draft?<form onSubmit={event=>{event.preventDefault();void saveEdit(claim)}}>
          ...
          <div className="claim-actions"><button disabled={busy||stale} onClick={()=>beginEdit(claim)}>Editar</button><button disabled={busy||stale||claim.status==='accepted'} onClick={()=>void onSelect(claim,'accept')}>Aceptar</button><button disabled={busy||stale||claim.status==='discarded'} onClick={()=>void onSelect(claim,'discard')}>Descartar</button></div></>}
      </article>)}</div>
    </>}
  </section>
}
